...
public class OrderSubmitAdapter extends BaseQuickAdapter<OrderCreateTmpBean.GoodsListBean,BaseViewHolder> {
    ...
    protected void convert(BaseViewHolder helper, OrderCreateTmpBean.GoodsListBean item) {
        Glide.with(context).load(item.getMain_pic()).into((ImageView) helper.getView(R.id.buy_iv));
        helper.setText(R.id.buy_tv_content,item.getGoods_name())
                .setText(R.id.buy_tv_price,"￥" + item.getUser_price())
                .setText(R.id.buy_tv_num,"X" + item.getGoods_number());
        if (item.getAttr() != null && item.getAttr().size() > 0) {
            helper.setText(R.id.buy_tv_specs, item.getAttr().get(0).getAttr_name() + ":" + item.getAttr().get(0).getValue());
        }
    }
}
