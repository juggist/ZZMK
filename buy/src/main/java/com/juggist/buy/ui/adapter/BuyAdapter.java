package com.juggist.buy.ui.adapter;
...
import com.juggist.buy.view.AddOrMinusButton;
...
public class BuyAdapter extends BaseQuickAdapter<ShopCarBean,BaseViewHolder> implements AddOrMinusButton.Listener {
    ...
    protected void convert(BaseViewHolder helper, final ShopCarBean item) {
        MyImageLoader.getInstance().loadImage(item.getMain_pic(),(ImageView) helper.getView(R.id.buy_iv));
        helper.setText(R.id.buy_tv_content, item.getGoods_name())
                .setText(R.id.buy_tv_price, item.getUser_price());
        if (item.getAttr() != null && item.getAttr().size() > 0) {
            helper.setText(R.id.buy_tv_spes, item.getAttr().get(0).getAttr_name() + ":" + item.getAttr().get(0).getValue());
        }
        if (selectArray.contains(item)) {
            helper.getView(R.id.buy_iv_select).setSelected(true);
        } else {
            helper.getView(R.id.buy_iv_select).setSelected(false);
        }

        helper.getView(R.id.buy_iv_select).setOnClickListener(new View.OnClickListener() {
            @Override
        ...
        ((AddOrMinusButton) helper.getView(R.id.buy_addOrMinus)).init(helper.getLayoutPosition(), item.getGoods_number(), this);
    }
    ...
    public void delete(final int position) {
        ...
        if (context != null) {
            new AlertDialog(context).builder()
                    .setTitle(context.getResources().getString(R.string.buy_toast_delete_product))
                    .setNegativeButton(context.getResources().getString(R.string.common_cancle), new View.OnClickListener() {
                        @Override
                        public void onClick(View v) {

                        }
                    })
                    .setPositiveButton(context.getResources().getString(R.string.common_ensure), new View.OnClickListener() {
                        @Override
                        public void onClick(View v) {
